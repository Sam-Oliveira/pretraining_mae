...
def plot_loss(train_data, labels):
    colors = {'Animals': 'red', 'Non-animals': 'green', 'Mixed': 'blue'}
    ...
    plt.tight_layout()
    plt.savefig('final_finetuned_loss_plot.pdf')
    plt.close()
...
def plot_accuracy(train_data, labels):
    ...
    plt.yticks(fontsize=14)
    plt.tight_layout()
    plt.savefig('final_finetuned_accuracy_plot.pdf')
    plt.close()
...
def plot_iou(train_data, labels):
    ...
    plt.yticks(fontsize=14)
    plt.tight_layout()
    plt.savefig('final_finetuned_iou_plot.pdf')
    plt.close()
